...
The SVG map therefore remains the production default until the table/board composition is inspected on real iPhone/iPad and Android hardware and both physical camera and battle profiles pass Gate 4. The deployed Expo alias was not changed, and no new signed release or App Store Connect submission was produced for this patch. The signed Android versionCode 11 AAB and signed iOS buildNumber 11 IPA remain historical distribution evidence that predates the patch; App Store Connect accepted build 11 through submission `2781fb7c-1e31-4da1-adcb-f6e9bca60474`, but TestFlight installation is not claimed. The current release gate remains 45/47; external work still includes a Google Play service-account key, physical Android/iOS R3F proof, two physical devices or a second authenticated human account for end-to-end multiplayer proof, and TestFlight processing/installation verification. Keep SceneKit as reference material only.
