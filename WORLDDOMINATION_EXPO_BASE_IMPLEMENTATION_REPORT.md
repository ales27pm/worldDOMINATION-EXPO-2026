...
- `pnpm test` currently runs both the compiled Node unit lane and Vitest lane, covering 130 Node tests and 2 Vitest tests.
...
The exact source passes TypeScript, 130 Node tests, 2 Vitest cases, deterministic asset validation, and the complete Playwright/export matrix with 152 exported files and 97 game images. The browser proof includes classic 42/42 and expanded 48/48 pick/label parity, a nonblank table scene, same-revision `3D -> 2D -> 3D` switching, Brazil-to-North-Africa attack selection, modal suspension, camera/battle profiles, evidence persistence/export, save/restore, and multiplayer snapshot presentation. The final local Android x86_64 build and the file-identical SSH/Xcode simulator build both pass. Simulator profiles and captures remain diagnostic only: Android's table profile misses strict p95/within-budget thresholds, iOS metrics are simulator-ineligible, and CoreSimulator's frozen GL capture prevents current iOS visual-parity proof.
