...
`scripts/browser-smoke.js` now checks the exact computed alpha values for the passive, control, roster, and dispatch surfaces as well as the camera controls. The full rendered smoke passes across portrait, landscape, and desktop, including 97 game images. TypeScript, 90 Node unit tests, 2 Vitest tests, and `git diff --check` pass. Review screenshots are stored in `.local/transparent-map-hud-20260729/`.
...
2. Generate a GLB from the canonical map geometry with one named mesh per territory and stable territory IDs.
