...
Last verified advancement: 2026-07-31
...
- Graph JSON: `docs/consolidation/worlddomination-complete-graph.json`
- Interactive graph: `docs/consolidation/worlddomination-complete-graph.html`
- Spreadsheet workbook: `docs/consolidation/worlddomination-complete-graph.xlsx`
...
As of the current 2026-07-31 working tree, the EXPO base has advanced beyond the initial consolidation plan. The implementation remains centered on `worldDOMINATION_EXPO/artifacts/mobile`; donor provenance and frozen revisions are recorded in `docs/CONSOLIDATION.md`, and the donors are historical remotes rather than alternate production targets.

### 2026-07-31 Same-Source R3F Gate 4 Acceptance

This section supersedes the earlier same-source pairing blockers retained in the dated implementation history below.

- Exact candidate source `391b4949ec67330cd6e390681a1be34de7bdffeb` was built and exercised independently on an Android 15 emulator with host GPU acceleration and a physical iPhone 16 Pro running iOS 26.6.
- Both records use application `1.0.0`, the canonical classic 42-territory board fixture, distinct session IDs, and the byte-identical scene fingerprint `13236:383488f9`.
- Android camera profiling recorded 76 samples over 1,266.644 ms at 60.001 average FPS with 16.834 ms p95 and zero estimated drops. Its battle profile recorded 124 samples over 2,066 ms at 60.019 average FPS with 17 ms p95/p99/max and zero estimated drops.
- Physical iOS camera profiling recorded 77 samples over 1,283.578 ms at 59.989 average FPS with 16.773 ms p95 and zero estimated drops. Its battle profile recorded 124 samples over 2,067 ms at 59.990 average FPS with 17 ms p95/p99/max and zero estimated drops.
- The unmodified release contract version 2 accepts the Android simulator metric pass and requires physical iOS. `pnpm run map:release:check` returns `status=pass` with an empty failure list for this pair.
- Canonical decision evidence is checked in at `docs/evidence/release-pair-391b494.json`; the two source records and full-board captures are indexed under `docs/evidence/android-emulator` and `docs/evidence/physical-ios`.

Gate 4 is therefore complete. The follow-on release change makes R3F the campaign and multiplayer production default while preserving `renderer=svg` and `renderer=2d` as explicit fallbacks. `GameMap` also retains its renderer error boundary, so an R3F initialization failure still falls back to SVG. The performance records remain truthfully tied to `391b494`; the default switch is a dedicated subsequent source change that must pass fresh static builds and platform smoke validation but does not rewrite qualification provenance.

The production-default working tree passes TypeScript, 148 Node tests, 2 Vitest cases, deterministic classic/expanded GLB regeneration and Khronos validation, and the complete Playwright browser matrix. The browser export contains 152 files and loads 97 game images. A renderer-less campaign route proves a nonblank default R3F tabletop with 42/42 pick meshes and labels; explicit SVG routes retain the standard and expanded 2D layout, camera, overlay, and save/restore checks.

### 2026-07-31 Android Emulator Acceptance

This policy supersedes every physical-Android-only R3F map qualification statement in the dated sections below.

- Gate contract version 2 accepts either Android emulator or physical Android evidence, while iOS remains physical-only. The environment waiver is deliberately narrow: exact full-SHA source provenance, app/build/session provenance, matching app version and canonical scene fixture, distinct sessions, evidence age and capture skew, correct 42/48 territory count, and passing camera plus battle profiles remain mandatory.
- `pnpm run map:release:check` is the canonical CI command. `map:physical:check` remains as a compatibility alias, but both execute the same release-pair verifier and publish the accepted environment policy in their JSON report.
- Exact source `efcbb0ad90aa28f8d4a4a0c31887495db7e65f47` now has a conforming Android emulator record. A clean x86_64 release build completed 529 Gradle tasks and produced a 52,434,238-byte APK with SHA-256 `b474512c3921f5ccf182407479309b25c26f562c3bd2ce9530024f67fe930475`, package `com.worlddomination.app`, version `1.0.0`, native build `1`, and target SDK 36. The build embedded the exact full Git revision; runtime app/build provenance now comes from `expo-application`, so the installed native binary reports its real build version.
- Android 15 emulator interaction covered deployment, full-board framing, R3F raycast selection, Argentina-to-Brazil attack targeting, battle resolution, repeated warmed battle presentation, zoom, and full-board restoration. The final record passes both profiles: camera measured 78 samples over 1,299.645 ms at 60.016 average FPS with 16.890 ms p95 and no estimated drops; battle measured 124 samples over 2,067 ms at 59.990 average FPS with 17 ms p95 and no estimated drops. The first cold battle is retained separately because one 126 ms first-use frame caused that record to fail.
- Versioned Android evidence is checked in under `docs/evidence/android-emulator`: `cold-fail.json`, `control-sweep-pass.json`, `r3f-battle-ready.png`, and `r3f-full-board-table.png`. The final image visibly confirms the unobstructed board seated on the round walnut table.
- Pairing the accepted Android record with the retained physical-iOS record now fails only `source-revision` on iOS and `scene-fixture-pair`; there are no Android environment, provenance, profile, device, application-version, age, or capture-skew failures. Gate 4 therefore requires a fresh physical-iOS run from the same source and canonical scene fixture, not physical Android hardware.
- Current source validation passes TypeScript, all 146 Node tests, both Vitest cases, deterministic GLB validation, and the label-atlas check. A fresh Expo web export produced 147 assets plus separate 2.57 MB entry and 1.05 MB lazy R3F chunks. The managed static Expo Go builder now requests Metro's Hermes transform profile for its JavaScript launch bundles, avoiding Three's unsupported default-profile static-class transform while preserving asset URL rewriting; the repaired build generated and verified both platform bundles, 145 files, one world map, all three piece assets, and 48 battle views.
- Exact source `864d9cfecaa8704c3aac06ac2e4194e3c7c381e2` closes a renderer-specific China regression discovered after that performance capture. SVG already covered the obsolete split/white China baked into the raster with the canonical territory path; R3F still sampled the stale raster directly. The shared R3F territory surface now suppresses that texture only for China, applies the corrected atlas fill, and draws one authoritative outer contour from the canonical geometry. Three focused unit tests prevent texture or border regression, and the complete Playwright/export matrix passes with 152 exported files and 97 game images. Browser and native Android captures show one continuous China region with no inland seam.
- A forced all-ABI Android release rebuild from `864d9cf` completed 565 Gradle tasks and produced a 114,503,487-byte APK with SHA-256 `3090417da7f5ffc1b3a421e92e189ef847b097adc100ef97f8347301e84fbeda`. The Android 15 emulator installed and ran that exact bundle in R3F mode; the board, round table, pieces, labels, attention control, zoom controls, and full-board framing rendered, and the focused runtime log contained no React Native, Expo GL, or Android fatal error. `docs/evidence/android-emulator/r3f-china-parity-864d9cf.png` records this source-parity smoke. It does not replace the accepted `efcbb0a` performance profiles.
- The same `864d9cf` source was synchronized file-for-file to `/Users/ales27pm/worlddomination-risk-scan-ios-build` and validated only through the required SSH macOS/Xcode path. Remote Node 22 TypeScript passed, CocoaPods 1.17 installed `EXApplication` 7.0.8, and the Xcode 26.3 Release simulator build completed for the booted x86_64 iPhone 16 with zero compiler or disk errors. The 55,552 KiB app reports bundle `com.worlddomination.app`, version `1.0.0`, build `1`, and minimum iOS `15.1`; its executable SHA-256 is `8e918686a73b47cfdf0a69588ffbffe2c62178bfeb12cfbdb6bef27d5eec464e`. The Hermes bundle contains the exact source revision and both corrected China material values. It installed and launched on iOS 26.3.1 without a JavaScript fatal, invariant, crash, termination, or new diagnostic report. The authoritative 9,239,665-byte compile log is `/tmp/worlddomination-ios-864d9cf-x86.log`, SHA-256 `e495cbf8047029d3f6055bb94614536e8b0387da6fa839dda506d96337a27299`. Headless `simctl` capture remained on a stale SpringBoard frame, so this is compile/bundle/launch evidence rather than iOS visual or physical-device qualification.
- Physical Android hardware is deferred and is not required for the current R3F renderer decision. This does not waive the separate multiplayer end-to-end proof, which still requires two real participants represented by two physical devices or a second authenticated human account.

### 2026-07-30 Live Release Advancement

This remains the authoritative release-artifact snapshot. The 2026-07-31 section supersedes its physical-Android-only renderer-gate policy; older URLs, artifact versions, credential conclusions, readiness totals, and notification-hosting notes later in the report are retained as dated history.
...
- Git source authority is consolidated on `https://github.com/ales27pm/worldDOMINATION-EXPO-2026`, branch `main`. The production tree retains the Expo app, multiplayer API, shared contracts/database code, release checks, crawl graph, and sanitized physical evidence. Replit metadata, raw attachments, duplicate donor files, the preview-only mockup workspace, committed build output, and the unused Replit-sidecar object-storage route are excluded.
- The consolidated seven-package workspace passes TypeScript, 143 Node unit tests, 2 Vitest cases, all 37 API authority tests against the available real `DATABASE_URL`, deterministic classic/expanded GLB validation and rendering smoke checks, and all 19 static release-readiness checks. A fresh web export produced 147 assets and two bundles. A fresh local Android `:app:assembleDebug` completed 369 tasks for all four ABIs in 5 minutes; its 206,666,469-byte APK has SHA-256 `3be0a0d092b01ea05bc55a86c4e9a2110a8b6d6e540a4e719bcd0466848b16ff`. This APK is current compile evidence only, not a distribution artifact. Generated `.local` paths named later are dated audit records rather than committed source; cleanup retains only ignored operational environment files, while the irreplaceable sanitized physical-iOS records are versioned under `docs/evidence/physical-ios`.
- Canonical commit `a76cfb77e4e228f7c24888b40202dad7d2f94c18` also passes a clean iOS validation from a fresh clone of `worldDOMINATION-EXPO-2026` on the required SSH macOS host. macOS 15.7.7, Xcode 26.3, Node 22.13.1, CocoaPods 1.17.0, frozen pnpm dependencies, remote TypeScript, Expo iOS prebuild, and all 106 installed pods completed successfully. The final one-job Xcode simulator retry ended with `** BUILD SUCCEEDED **` and zero compiler error or disk-failure lines after a two-job attempt was interrupted for an idle ExpoModulesCore compatibility-header process. The 114,892 KiB x86_64 `worldDOMINATION.app` reports bundle `com.worlddomination.app`, version `1.0.0`, build `1`, minimum iOS `15.1`, and executable SHA-256 `4805a37570292bbc3a0c4e27ad101809695cbe4bd6368c765faddbdb76432a15`. It installed and launched on an isolated iPhone 16 / iOS 26.3 simulator, Metro served the same checkout's 1,860-module bundle, the worldDOMINATION home screen rendered, and the focused runtime log contained no fatal JavaScript, invariant, crash, or termination signal. The copied validation log is `/tmp/worlddomination-canonical-a76cfb7-xcode-retry-20260730.log` (12,065,738 bytes, SHA-256 `b35314804b6bade6a79a8540bdcd8e9f1c21631b448ae6d8feafd347ee4784f2`); the rendered capture is `/tmp/worlddomination-canonical-a76cfb7-ios-dedicated-ready.png` (1,956,617 bytes, SHA-256 `01d0244a75b4e5a0292fac081810738ff5a7a331188fdc67b4d523aad00d7855`). This proves canonical unsigned simulator compilation and launch, not store signing, physical-device qualification, TestFlight, or App Store submission.
...
- Physical-device enumeration was asymmetric during qualification. The QEMU-attached Apple USB device with serial suffix `C0801C` was paired and available to the SSH macOS host as an iPhone 16 Pro (`iPhone17,1`); local ADB exposed no physical Android device. After evidence capture the USB transport disconnected, so the current live recheck finds no Apple mobile USB device and CoreDevice retains only a cached unavailable entry. Physical iOS R3F evidence remains available, but physical Android, two-device, and two-user proof remain unavailable.
...
- Post-submission R3F contract hardening now builds one shared `MapSceneModel` at the renderer boundary and feeds that exact model to both SVG and R3F, with revisions covering every renderer-facing field. It also emits each newly observed battle presentation once, suppresses restored or repeated battle snapshots, resolves equal-depth border hits by canonical territory order, repairs native demand-render pick registration, and records renderer-neutral active camera/battle frame profiles against a fail-closed 60 Hz qualification contract. Complete evidence now carries app/source/device/scene provenance, persists under a versioned key, and can be exported through the native share sheet. Browser and simulator runs report metric results with the renderer-level status `ineligible`; release-pair contract version 2 now accepts passing Android simulator metrics while still excluding browser and iOS Simulator evidence. Complete passing physical profiles remain accepted on both platforms. R3F territory-name parity comes from a deterministic 1024x1024 IM Fell English atlas rendered as one merged, depth-aware mesh; classic and expanded scenes report 42/42 and 48/48 labels respectively. The exact patch passes TypeScript, 130 Node tests, 2 Vitest tests, the complete Playwright/export matrix including a same-revision live `3D -> 2D -> 3D` switch and browser-ineligibility assertion, native Android interaction and compilation, real iOS 26.3 Simulator interaction and qualification through the SSH macOS host, and fresh SSH/Xcode simulator compilation. The signed versionCode 11 AAB and buildNumber 11 IPA above predate this patch and remain historical distribution evidence; they are not claimed to contain the patch.
- The current table-and-attention follow-up keeps the installed Expo 54 / React Native 0.81 stack and adds a deterministic `MapAttentionDirector` rather than upgrading the runtime. Cached per-territory bounds feed renderer-neutral camera intents; 120 ms coalescing, priority, TTL, duplicate suppression, a 400 ms automatic-move floor, a three-move/ten-second budget, depth-counted gesture veto, and a 2.5-second manual lease prevent camera theft. Explicit taps can resume attention during that lease. Accessibility announcements, screen-reader/reduced-motion suppression, background/modal cancellation, and the existing 3D highlight layer remain independent of camera movement. A tap now caches its raycast target before RNGH's double-tap exclusion delay, so an intervening camera transition cannot reinterpret the touched territory.
- `R3FTable.tsx` now places the board into a dark-walnut round tabletop with apron, trim, central pedestal, and four curved feet derived from the supplied table reference. Full-board framing includes a stable tabletop reveal in portrait and landscape. Native rendering stays `demand` when idle, runs continuously only while camera work must be presented, settles two native frames before release, and remains `never` behind the battle modal. The final browser export produced 152 files and the complete Playwright matrix passed with 97 game images, including the rapid Brazil-to-North-Africa raycast sequence. The exact source also passes the x86_64 Android Debug compile and the SSH-only Xcode simulator compile. These are development-build and simulator proofs; the signed versionCode 11 AAB and buildNumber 11 IPA still predate this follow-up.
- The cross-platform handoff is an executable fail-closed release gate. EAS qualification builds require committed source and embed the exact full Git revision through `EAS_BUILD_GIT_COMMIT_HASH`; the accepted local Android qualification build explicitly injected the same full revision. `pnpm run map:release:check` accepts one parsed evidence file per platform and passes only when recent Android emulator-or-hardware evidence and physical-iOS evidence use the expected commit, the same application version and canonical scene fixture, distinct runtime sessions, credible environment-specific device/OS identities, correct 42/48 territory counts, and passing camera plus battle profiles. The old `map:physical:check` command is retained as an exact compatibility alias. Exact source `efcbb0ad90aa28f8d4a4a0c31887495db7e65f47` now has passing Android emulator evidence with native build provenance. The retained physical-iOS record is independently passing but comes from `0c136a5c57e5fd0d9bd68946fbf2244da29622ce` and a different scene fixture, so the paired gate still exits nonzero and SVG remains the production default.
- A clean local EAS `preview` build from exact source `0c136a5c57e5fd0d9bd68946fbf2244da29622ce` produced the installable ad hoc IPA `.local/eas-builds/worlddomination-preview-r3f-physical-ios-0c136a5.ipa`, buildNumber `15`, 22,287,407 bytes, SHA-256 `42d35d724ce58d9e839922f2263976819807cac57ec614270b8be1b14181b253`. The embedded profile includes the registered iPhone, strict deep code-sign verification passes, the release bundle embeds the exact source SHA, and the canonical classic/expanded GLBs remain 980,496/1,019,592 bytes. CoreDevice installed and launched the IPA on the paired iPhone 16 Pro running iOS 26.6 beta.
- A separately signed XCTest/WebDriverAgent runner then exercised the real app without changing its source. Native input launched and continued a campaign, switched `2D -> 3D`, selected China through the R3F raycaster, changed reinforcements from 4 to 3, restored that exact saved state after relaunch through `Continue Campaign`, placed the remaining armies, selected China as attacker, targeted Mongolia, resolved two battle effects, and exercised territory-attention focus, zoom-in, zoom-out, and full-board controls. The full-board capture at `docs/evidence/physical-ios/r3f-full-board-table.png` visibly shows the canonical board seated on the round walnut table; the companion board and battle captures are retained beside it.
- The first cold battle produced a complete physical record with camera pass but aggregate fail: battle measured 121 samples over 2,058 ms at 58.795 average FPS, 17 ms p95, 22 ms p99, and two estimated dropped frames, so its 1.7% dropped-frame ratio exceeded the 1% limit. A second warmed battle passed, and the final control-sweep record remained `status=pass`: camera measured 78 samples over 1,296.829 ms at 60.147 average FPS, 16.747 ms p95, no estimated drops, and 1.000 within budget; battle measured 123 samples over 2,051 ms at 59.971 average FPS, 17 ms p95/p99, 18 ms max, no estimated drops, and 1.000 within budget. The cold and final JSON records are `docs/evidence/physical-ios/cold-fail.json` and `docs/evidence/physical-ios/control-sweep-pass.json`, with SHA-256 `993031ed753c3b0a2ebdea5c780370a789472690e747f752dac1450434e8a396` and `2378f478be464142f6382b2bbaa6b26495c645413ab137bc4c597c718f3f1feb`. The checked-in parser accepts both and reproduces their fail/pass assessments.
...
- Physical-device enumeration found the QEMU-attached, paired iPhone 16 Pro on the SSH macOS host for the qualification run; local ADB found no physical Android device. The phone disconnected after evidence capture and is not currently attached. This supplies one-device iOS evidence only, so no physical multi-device claim is made.
...
- `pnpm test` currently runs both the compiled Node unit lane and Vitest lane, covering 137 Node tests and 2 Vitest tests.
...
| Native Android debug binary compile | Verified | `ANDROID_HOME=/home/ales27pm/.local/android-tooling/android-sdk-1785090272 ANDROID_SDK_ROOT=/home/ales27pm/.local/android-tooling/android-sdk-1785090272 JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64 ./gradlew :app:assembleDebug -PreactNativeArchitectures=x86_64 --no-daemon --max-workers=2 --console=plain` completed successfully from `artifacts/mobile/android`. Gradle explicitly autolinked `expo-device` 8.0.10. The exact-source x86_64 debug APK is `artifacts/mobile/android/app/build/outputs/apk/debug/app-debug.apk`, 74,508,366 bytes, SHA-256 `3d42538062e1ea15b810ea7cef6b0e41e62c19a5222978a2c850fafdf96f58b8`. | This proves an ABI-limited Android emulator Debug compile, separate from the all-ABI EAS release AAB evidence below. |
| Native iOS/Xcode simulator binary compile | Verified | Xcode work ran through `ssh -p 2222 ales27pm@127.0.0.1` on macOS with Xcode 26.3. The exact synchronized source passed remote Node 22 TypeScript validation, CocoaPods linked `ExpoDevice` 8.0.10, and `xcodebuild -quiet -workspace ios/worldDOMINATION.xcworkspace -scheme worldDOMINATION -configuration Debug -sdk iphonesimulator -destination "platform=iOS Simulator,id=710EFEFE-73D8-400B-B5D1-D5F43662B536" -derivedDataPath ios/build/DerivedData -jobs 2 ONLY_ACTIVE_ARCH=YES COMPILER_INDEX_STORE_ENABLE=NO CODE_SIGNING_ALLOWED=NO build` exited 0 with no logged errors. The current output is `/Users/ales27pm/worlddomination-risk-scan-ios-build/artifacts/mobile/ios/build/DerivedData/Build/Products/Debug-iphonesimulator/worldDOMINATION.app`, 114,868 KiB, with an x86_64 simulator executable, bundle id `com.worlddomination.app`, and executable SHA-256 `4805a37570292bbc3a0c4e27ad101809695cbe4bd6368c765faddbdb76432a15`. A live simulator run published completed camera and battle frame profiles plus the required `simulator`/`ineligible` aggregate qualification. | This proves unsigned Debug simulator compilation and fail-closed simulator runtime qualification through SSH/Xcode, not signed physical-device install, physical 60 fps qualification, TestFlight, or App Store readiness. |
...
| Exact-source R3F gate distribution and physical iOS artifacts | Partial | This row supersedes the historical distribution artifacts above for source `0c136a5c57e5fd0d9bd68946fbf2244da29622ce`. Local EAS production Android produced `.local/eas-builds/worlddomination-production-r3f-gate4-0c136a5-android.aab`, versionCode 12, 77,598,062 bytes, SHA-256 `ca69c5e6837222620fe5f2f58bdd7754cac92ad1bdbbeb389a2d480f43c642fc`; `jarsigner -verify` passes and the established signer fingerprint is unchanged. Local EAS production iOS through SSH/Xcode produced `.local/eas-builds/worlddomination-production-r3f-gate4-0c136a5-ios.ipa`, buildNumber 14, 28,491,028 bytes, SHA-256 `30ecfc1260d5c550b5ee1e08fc887779ed8a7b64d751af625aaccfef866d95ea`; strict code-sign verification passes for Team `52T7P32J34`. EAS submission `a491d19e-871e-42ed-ac5e-0732cd41f6b1` successfully uploaded that exact IPA to App Store Connect app `6796113966`, where Apple accepted it for processing. An exact-source ad hoc buildNumber 15 IPA is 22,287,407 bytes with SHA-256 `42d35d724ce58d9e839922f2263976819807cac57ec614270b8be1b14181b253`; it was installed and driven on a real iPhone 16 Pro. Its final persisted camera and battle qualification passes with no missing kinds. | This proves exact-source signed Android/iOS artifacts, iOS upload, physical iOS installation/interaction, and a passing physical iOS R3F profile. It does not prove Google Play upload, Apple processing completion, TestFlight availability/install, a passing Android emulator-or-hardware qualification, or the required same-source Android+iOS evidence pair. |
...
The latest successful Android debug artifact is the x86_64 emulator APK at `worldDOMINATION_EXPO/artifacts/mobile/android/app/build/outputs/apk/debug/app-debug.apk`, 74,508,366 bytes, SHA-256 `3d42538062e1ea15b810ea7cef6b0e41e62c19a5222978a2c850fafdf96f58b8`.
...
- `game/mapAttentionDirector.ts`, which arbitrates explicit and automatic focus requests with priority, coalescing, TTL, duplicate suppression, an automatic-move budget, gesture cancellation, and a manual-control lease. Its target registry caches GLB geometry bounds after load and computes a single-territory or multi-territory camera fit for the current aspect without repeating geometry traversal per frame.
- `game/mapRenderLoop.ts`, which keeps web instrumentation continuous, holds native rendering only while camera presentation is active, returns an idle native scene to `demand` after two settle frames, and preserves modal suspension.
...
- `game/mapFrameQualification.ts`, which requires both camera and battle reports, evaluates them against versioned 60 Hz thresholds with deterministic failure codes, and separates metric pass/fail from physical-hardware eligibility. `expo-device` identifies native physical hardware; web is explicitly classified as `browser` because its `isDevice` value is not a native-device signal.
- An R3F tabletop that loads the classic or expanded canonical GLB and painted atlas, renders named extruded territory meshes plus transparent raycast meshes, and adds selection/target halos, lighting, and shadows. The board now sits inside a procedural round walnut table with a stable full-board reveal across tested aspects.
...
The exact final source passes TypeScript, 130 Node tests, 2 Vitest tests, deterministic GLB and territory-label atlas regeneration, and the Khronos validator with zero GLB errors or warnings. The checked-in label atlas is 250,218 bytes with SHA-256 `39a08d962e2eca221a99a3b3f6a0c306447b74e9a62a29f6a2e15880e775c11f`; its manifest records the exact font hash and canonical label indices. The complete Playwright browser matrix passes across portrait, landscape, and desktop with 97 game images. Its R3F routes prove scene readiness, 42/42 classic pick meshes and labels, 48/48 expanded pick meshes and labels, a nonblank WebGL canvas, the three army classes, camera zoom, completed camera and battle frame profiles, Brazil and North Africa raycasts, attack dispatch, modal suspension, finite effect completion, versioned evidence persistence/export eligibility, canonical save/restore without replay, and one-shot rendering of a changed multiplayer snapshot without replay on identical polling snapshots. It also requires `environment=browser`, `status=ineligible`, no missing profile kinds, and contract version 1, proving browser metrics cannot satisfy the physical gate. The browser switches live from R3F to SVG and back on the same game route, requires the identical shared scene revision and territory count in both renderers, verifies R3F unmount cleanup, and proves the remounted scene restores all 42 pick meshes and labels before continuing the interaction suite. The inspected classic browser frame is `.local/release-evidence/r3f-labels-web-classic-20260730.png`. Software SwiftShader is sufficient for functional smoke and qualification-contract validation but is not valid evidence for the Gate 4 60 fps target-device requirement.

The native renderer now uses a platform split rather than sending Three.js `GLTFLoader` through Expo GL. Web retains `GLTFLoader` and the WebP atlas. Native resolves Expo assets into a validated GLB parser, uses the 1152x768 RGB PNG atlas required by the native texture path, disables expensive dynamic shadows in favor of deterministic instanced contact shadows, and runs the visible R3F canvas on demand at DPR 1.25. The shared camera contract explicitly invalidates frames during gestures and motion. Camera actions expose selected state correctly: focus selects the focus control, while full board clears it. Their hit targets sit above the map interaction surface.

The camera path is now an attention contract rather than a generic autofocus. Territory taps request a priority-80 fit from cached geometry bounds; battles request the union of attacker and defender at priority 75. Pan and pinch activation cancel current and pending automatic motion immediately, use a depth counter for simultaneous gestures, and retain manual ownership for 2.5 seconds after the final gesture. Full-board, zoom, wheel, and double-tap camera commands also claim manual ownership. A double-tap must remain within 24 points, while every prospective tap caches the territory hit against the camera pose present under the finger; this prevents the delayed single-tap recognizer from raycasting against a camera that moved during recognition. App backgrounding and battle modals cancel attention. Screen readers and reduced-motion settings retain native descriptions and explicit-user focus while suppressing automatic game travel.

The battle-modal interaction contract is now platform explicit. While a React Native battle modal covers the map, the R3F `frameloop` is `never`; active-frame cursors reset on every shared visibility revision, so covered wall time cannot enter camera or battle measurements. After dismissal, web returns to its normal loop and native returns to `demand`. iOS waits for `Modal.onDismiss` before clearing shared visibility, then creates a fresh Canvas because the existing Expo GL root does not reliably repaint after UIKit modal dismissal. Completed profiles are owned above that root and survive the recreation. React Native's Android modal does not dispatch `onDismiss`, so Android clears shared visibility on the next task after setting `visible=false` and resumes the same paused Canvas. Recreating the Android Canvas produced overlapping Expo GL roots and a reproducible Hermes Hades collector fault; the iOS-only recreation avoids that failure. The finite battle effect self-invalidates for 2.05 seconds and then returns native rendering to idle demand mode.

`game/mapPerformanceEvidence.ts` adds a versioned portable evidence schema with capture time, platform, app/build/source/session provenance, device identity, scene contract/revision/territory count, both raw reports, deterministic assessments, and aggregate qualification. `lib/mapPerformanceEvidence.ts` rejects malformed or incomplete records and prevents a partial profile from replacing complete evidence. `CampaignScreen` loads prior complete evidence, persists each complete update, shows pending/pass/fail/ineligible state, and exports the formatted JSON through the platform share surface. The EAS `preview` profile enables qualification explicitly; production remains unchanged. Browser smoke verifies persistence, export enablement, covered-modal suspension, valid battle outcomes, save restore, multiplayer snapshots, and Same Time pacing.
...
The final post-instrumentation Android 15 x86_64 emulator run supersedes the earlier timing sample. It started from cleared app data and the R3F attack fixture, completed a full-board camera action, selected Brazil and North Africa through native raycasts, held the battle modal at `TAP TO ROLL` for more than 11 seconds, and proved directly from `RKStorage` that no evidence was written while the scene was covered. After a real dice roll and modal dismissal, the same Canvas resumed, the finite battle effect completed, the process remained foreground, complete evidence persisted, and Android's system chooser rendered the shared JSON. Camera measured 75 samples over 1,249.985 ms: 60.001 average FPS, 16.694 ms p50, 20.447 ms p95, 25.526 ms p99/max, one estimated drop, and 0.867 within budget. Battle measured 123 samples over 2,038 ms: 60.353 average FPS, 17 ms p50, 20 ms p95, 23 ms p99, 24 ms max, no estimated drops, and 0.911 within budget. The aggregate has no missing kinds but reports `metricStatus=fail` for p95/drop/within-budget violations and `status=ineligible` because the environment is a simulator. Evidence is `/tmp/worlddomination-android-evidence-final.json`; the chooser capture is `/tmp/worlddomination-android-share-final.png`. Final native compilation also passes `:app:assembleDebug` for x86_64. These functional and timing results do not satisfy the physical Android gate.

The table-and-attention follow-up was then run against exact Metro source on the same Android emulator. The clean full-board capture `/tmp/worlddomination-android-table-full-clean.png` shows the canonical board seated inside the round walnut tabletop. Its warmed camera profile reported 60.002 average FPS, 16.673 ms p50, 18.952 ms p95, 19.411 ms p99, no estimated drops, and 0.903 within budget. This simulator profile remains both metric-failing under the strict p95/within-budget contract and hardware-ineligible. The final x86_64 `:app:assembleDebug` completed 333 tasks in 15 seconds; `app-debug.apk` remains 74,508,366 bytes with SHA-256 `3d42538062e1ea15b810ea7cef6b0e41e62c19a5222978a2c850fafdf96f58b8`.

The current exact-source store-signed local EAS Android production build completed in 5m36s from clean commit `0c136a5c57e5fd0d9bd68946fbf2244da29622ce` and produced `.local/eas-builds/worlddomination-production-r3f-gate4-0c136a5-android.aab`, 77,598,062 bytes, SHA-256 `ca69c5e6837222620fe5f2f58bdd7754cac92ad1bdbbeb389a2d480f43c642fc`, versionCode 12. `jarsigner -verify` reports `jar verified`, and the established production signer SHA-256 fingerprint remains `97:7D:8A:82:C3:BF:95:F8:54:DB:AE:4A:50:2E:A4:D6:FB:3F:F7:B8:07:AC:A8:A0:3A:D9:57:95:44:AE:D8:D8`. Bundletool confirms `com.worlddomination.app` version `1.0.0`, target SDK 36, and versionCode 12. Its Hermes bundle contains the exact full source SHA, Expo GL is present for all four Android ABIs, and the canonical classic/expanded GLBs remain 980,496 and 1,019,592 bytes. This signed AAB has not been uploaded to Google Play.

The current native iOS source also compiles through the required SSH macOS/Xcode path. The exact synchronized patch passed remote TypeScript under Node 22 and CocoaPods installed `ExpoDevice` 8.0.10. A first final-patch universal build failed while writing a generated dependency file because the macOS data volume was full; it reported `No space left on device`, not a source error. After an Xcode clean and deletion of unavailable simulator data recovered space, the Xcode 26.3 retry targeted the booted x86_64 simulator with `ONLY_ACTIVE_ARCH=YES`, two build jobs, and indexing disabled. It exited 0 with no logged errors or disk-space failures and produced a 114,868 KiB `worldDOMINATION.app` with an x86_64 executable, bundle `com.worlddomination.app` version `1.0.0`, and executable SHA-256 `4805a37570292bbc3a0c4e27ad101809695cbe4bd6368c765faddbdb76432a15`. The authoritative compile log is `/tmp/worlddomination-table-attention-xcode-20260730.log` on the macOS guest. The later physical-device work supersedes the earlier device-enumeration limitation: the SSH host now exposes the paired iPhone 16 Pro as available. The earlier signed local EAS production archive produced buildNumber 11 and submission `2781fb7c-1e31-4da1-adcb-f6e9bca60474`; App Store Connect accepted that earlier binary for processing, but it predates this patch.

The exact-source signed iOS distribution build now supersedes that historical buildNumber 11 artifact. A clean macOS checkout at commit `0c136a5c57e5fd0d9bd68946fbf2244da29622ce` completed local EAS production buildNumber 14 through the required SSH/Xcode path and produced `.local/eas-builds/worlddomination-production-r3f-gate4-0c136a5-ios.ipa`, 28,491,028 bytes, SHA-256 `30ecfc1260d5c550b5ee1e08fc887779ed8a7b64d751af625aaccfef866d95ea`. The IPA is `com.worlddomination.app` version `1.0.0`, minimum iOS 15.1, uses the App Store profile UUID `f7a45e62-432e-421f-931f-477f7be8c95f`, and passes `codesign --verify --deep --strict` under distribution Team `52T7P32J34`. Its bundle contains the exact full source SHA and both canonical GLBs at 980,496 and 1,019,592 bytes. EAS submission `a491d19e-871e-42ed-ac5e-0732cd41f6b1` successfully uploaded this exact IPA to App Store Connect app `6796113966`; Apple accepted the binary and reported that processing had started. Processing completion, TestFlight availability, and installation are not yet claimed.

The final no-smoke iOS run used a clean install on the headless iPhone 16 / iOS 26.3.1 simulator reached exclusively through SSH. CoreSimulator taps selected Brazil and North Africa through the real Gesture Handler/raycast path. The battle modal remained at `TAP TO ROLL` with no evidence while covered, then a real roll and dismissal recreated the Canvas only after UIKit `onDismiss`. The resumed battle effect and a post-recreation camera action completed both profiles; complete evidence persisted and the iOS share sheet rendered it. Runtime provenance reports `modelName=Simulator iOS`, `modelId=x86_64`, and `environment=simulator`. Camera measured 77 samples over 1,280.006 ms: 60.156 average FPS, 16.662 ms p50, 17.939 ms p95, 18.058 ms p99/max, zero slow frames or estimated drops, and 1.000 within budget. Battle measured 122 samples over 2,033 ms: 60.010 average FPS, 17 ms p50, 18 ms p95/p99/max, zero slow frames or estimated drops, and 1.000 within budget. Both assessments pass, so the aggregate is `metricStatus=pass`, but it remains `status=ineligible` with no missing kinds because the environment is a simulator. Evidence is `/tmp/worlddomination-ios-evidence.json`; the share capture is `/tmp/worlddomination-ios-share.png`. Earlier runtime inspection still supplies classic 42/42/42 and expanded 48/48/48 territory/pick/label registration plus same-revision renderer switching. The subsequent table-and-attention runtime supersedes that camera-state diagnosis: the JavaScript camera reached the centered full-board pose deterministically. Visual parity is still not claimed because CoreSimulator capture could not provide current GL pixels. These are simulator functional and fail-closed qualification results, not physical-iPhone or target-hardware evidence.

The exact table-and-attention source was synchronized file-for-file to `/Users/ales27pm/worlddomination-risk-scan-ios-build`; all 11 changed source/test SHA-256 values match the Linux checkout, remote Node 22 TypeScript validation passes, and the final Xcode 26.3 simulator build exits 0. The output remains a 114,868 KiB x86_64 `worldDOMINATION.app` for `com.worlddomination.app` version `1.0.0`, with executable SHA-256 `4805a37570292bbc3a0c4e27ad101809695cbe4bd6368c765faddbdb76432a15`; the authoritative guest log is `/tmp/worlddomination-table-attention-xcode-20260730.log`. Runtime camera profiles on the simulator remained approximately 60.5 FPS with p95 near 16.75 ms, no estimated drops, and a 1.000 within-budget ratio, but simulator evidence is ineligible. `simctl` still screenshots and frames extracted from a recorded movie froze the Expo GL IOSurface: all sampled frames were byte-identical while JavaScript camera state continued to change. The physical buildNumber 15 run supersedes that capture limitation for iOS: WDA screenshots show live GL pixels, the round table, China, units, camera framing, and battle results on the real iPhone.

The version 1 performance-profile contract targets 60 fps and requires both camera and battle profiles. Each profile requires at least 60 samples and 900 ms duration, average FPS at least 58.5, p95 no more than 18.333 ms, p99 no more than 33.333 ms, estimated dropped-frame ratio no more than 1%, and at least 95% of samples within the 16.667 ms frame budget. Deterministic failures identify invalid metrics, target mismatch, insufficient samples or duration, low average FPS, p95/p99 excess, dropped-frame excess, and within-budget deficit. Renderer-level qualification remains `pending` until both kinds exist, reports `ineligible` on browser or simulator, and returns `pass` or `fail` directly only on physical hardware. Release-pair contract version 2 deliberately interprets a complete Android simulator record by its `metricStatus` and profile assessments, while continuing to reject browser and iOS Simulator records.

`game/mapReleaseQualification.ts` composes those per-environment results into the version 2 Android+iOS release decision. It validates exact full-SHA source provenance, application/build/session provenance, credible environment-specific device identity, canonical territory counts, same-version and byte-identical scene revision pairing, evidence age, capture skew, and both required profile assessments. `scripts/verify-map-release-qualification.ts` exposes the decision as a CI-friendly JSON command with a nonzero exit on failure. `scripts/write-build-source-revision.cjs`, invoked by the EAS post-install hook, embeds the official build commit without committing a stale SHA to source. Running the release verifier against `docs/evidence/android-emulator/control-sweep-pass.json` and the retained physical-iOS record exits 1 with exactly two findings: the physical-iOS source revision differs from the requested `efcbb0ad90aa28f8d4a4a0c31887495db7e65f47`, and the records use different canonical scene revisions. The Android environment, application/build/session provenance, device identity, territory count, evidence age, capture skew, camera profile, and battle profile all pass.

The exact physical-gate source passes local and SSH-hosted Node 22 TypeScript validation, 137 Node tests, 2 Vitest cases, deterministic GLB/label checks, and the complete Playwright matrix with 152 exported files and 97 game images. Expo Doctor now passes 18/18 checks after generated `artifacts/mobile/.expo` metadata was removed from Git tracking while remaining locally ignored. The local x86_64 Android Debug compile completes 333 tasks successfully and preserves the 74,508,366-byte APK with SHA-256 `3d42538062e1ea15b810ea7cef6b0e41e62c19a5222978a2c850fafdf96f58b8`. The file-identical Xcode 26.3 simulator compile exits 0 with no logged source or disk errors and produces the 114,868 KiB x86_64 app for `com.worlddomination.app` version `1.0.0`; its executable SHA-256 remains `4805a37570292bbc3a0c4e27ad101809695cbe4bd6368c765faddbdb76432a15`. The authoritative guest log for this pass is `/tmp/worlddomination-gate4-xcode-20260730.log`.

The SSH macOS build host is reachable with macOS 15.7.7 and Xcode 26.3, and the real iPhone completed pairing. During qualification Linux enumerated Apple USB device `05ac:12a8` with serial suffix `C0801C`, QEMU owned the live USB node through stable passthrough, and `xcrun devicectl list devices` reported an available paired iPhone 16 Pro. Exact-source buildNumber 15 was signed, installed, launched, screen-captured, and driven through a dedicated XCTest runner. Real raycast selection, save/continue, attack targeting, finite battle presentation, zoom controls, and full-board table framing all completed. The final persisted physical iOS evidence passes both 60 Hz profiles. After evidence capture QEMU logged `NO_DEVICE`; the current live recheck finds no Apple USB device and CoreDevice lists only the cached iPhone as unavailable. Exact source `efcbb0ad90aa28f8d4a4a0c31887495db7e65f47` now supplies the matching-quality Android emulator half, and Android hardware is no longer required. The retained physical-iOS record remains tied to `0c136a5c57e5fd0d9bd68946fbf2244da29622ce` and a different scene revision, so a fresh physical-iOS capture from the current exact source and fixture is still required. Gate 4 is not open.
...
3. **Complete:** R3F selection, attack targeting, attention-directed camera, round-table presentation, three army models, finite battle animation, deterministic picking, save/restore, and multiplayer snapshot rendering pass web and native validation.
4. **Complete:** exact source `391b4949ec67330cd6e390681a1be34de7bdffeb` passes the versioned 60 fps qualification contract on an accepted same-source pair: Android emulator plus physical iOS. Both records use the identical canonical scene fingerprint, and the pair checker reports no failures.

## Production-Default Release Delivery (2026-07-31)

Canonical commit `b8e16b24fa9d717225c531a02ef1dc4fb0806b85` promotes R3F to the production default while preserving explicit `renderer=svg` and `renderer=2d` fallback routes. The commit is pushed to `main` at `https://github.com/ales27pm/worldDOMINATION-EXPO-2026.git`. All builds and runtime captures in this section use that exact source revision.

The local web export completed with 152 files. Its deploy dry-run archive is 16,536 bytes with SHA-256 `9aea72a2f8f116212dc980c79841d6dc4a45f7128f225048d7c088506ff7cb4a`. This validates the deployable web artifact but does not claim a hosted deployment.

Local EAS produced the signed Android production AAB for `com.worlddomination.app` version `1.0.0` (`versionCode` 14). It is 77,608,608 bytes with SHA-256 `ccc1a50e565b18aaec9cb3b00b443f2a085659052cbfeaec309fe9f24c97ce6c`. Bundletool validates the package, SDK levels, four ABIs, Expo GL payload, and exact canonical GLBs. The accepted Android emulator smoke starts in R3F, renders the corrected continuous China region, and passes pan, zoom, focus, full-board framing, and `3D -> 2D -> 3D` switching without fatal logs. Physical Android remains optional under the accepted release-pair contract. Google Play upload is not complete: EAS Submit staged the AAB but could not configure Play credentials non-interactively because no Google service-account JSON key is available. No Play submission was created.

The required SSH/Xcode path on macOS 15.7.7 and Xcode 26.3 produced and installed an exact-source Release device app on the paired iPhone 16 Pro. That direct build passed default-R3F, pan, zoom, focus, full-board, and renderer-switch smoke. Local EAS then produced the App Store IPA for `com.worlddomination.app` version `1.0.0` build 19. The IPA is 28,505,880 bytes with SHA-256 `249958e76edbe8d4c53a73a1610bce7e2b4c8bbb0cea99d31ff68a6cafcd57f6`; deep strict signature verification passes with the App Store distribution profile, and the exact source and both canonical GLBs are embedded.

EAS submission `dbc34fa2-a5eb-4024-86bd-8466a910585a` uploaded build 19 to App Store Connect app `6796113966`. Apple processed the build, TestFlight exposed `1.0.0 (19)`, and the physical iPhone replaced the development build with TestFlight build 19. `devicectl` independently reports installed bundle version 19. The store-signed app launched through its qualification deep link, exposed `map-3d-root` and `map-3d-canvas`, rendered a nonblank corrected R3F board, and completed full-board recenter on the round table.

The live Postgres-backed multiplayer service also passed a deterministic two-identity proof. A fresh exact match claimed one human seat under each of two distinct trusted identities, advanced through `ACKNOWLEDGE_HANDOFF`, applied a reducer-owned `DEPLOY`, and delivered version 4 with the same army count to both WebSocket clients. Host and joiner scoped lobbies contained the match, an unrelated identity received no scoped matches, and snapshots and lobby responses excluded both player tokens. The distinct-identity assertion ran over loopback against the same live process because Tailscale Serve replaces the trusted identity header with the single connected Tailscale account; tailnet HTTPS `/api/healthz` was verified separately. The sanitized proof and all production-default screenshots are committed under `docs/evidence/production-default-b8e16b2/`.

## Final Recommendation

Continue with `worldDOMINATION_EXPO/artifacts/mobile` as the only production target for the consolidation.

The implemented release now includes the corrected China and mode-specific West Africa atlas, the same single-region China correction in R3F, transparent map HUD, deterministic indexed classic/expanded named-territory GLBs, one shared renderer-boundary scene model consumed by SVG and R3F, a merged territory-name atlas, renderer-neutral camera intent, active-frame profiling, a platform-specific release qualification contract, and versioned persistent/exportable evidence. The R3F path seats the board inside the supplied round-table design and routes camera motion through cached territory bounds plus a deterministic Attention Director with coalescing, priority, TTL, duplicate suppression, automatic-move budgeting, gesture cancellation, a 2.5-second manual lease, reduced-motion/screen-reader behavior, and independent native announcements. Delayed tap recognition caches the original raycast result, so camera motion cannot reinterpret a rapid follow-up territory tap.

Exact candidate source `391b4949ec67330cd6e390681a1be34de7bdffeb` supplies the accepted Android-emulator and physical-iPhone performance pair. The browser proof includes classic 42/42 and expanded 48/48 pick/label parity, a nonblank table scene, same-revision `3D -> 2D -> 3D` switching, a corrected continuous R3F China region, Brazil-to-North-Africa attack selection, modal suspension, camera/battle profiles, evidence persistence/export, save/restore, and multiplayer snapshot presentation. The accepted native pair additionally proves camera control, battle presentation, turn handoff, full-board table framing, and byte-identical scene-fixture provenance on both platforms.

R3F is now the production-default renderer for campaign and multiplayer gameplay. Explicit `renderer=svg` or `renderer=2d` routes and the runtime error boundary retain the SVG implementation as the controlled fallback. The accepted qualification pair is complete and physical Android hardware is not required by the current contract. Web, Android build/runtime, physical iOS, App Store Connect processing, TestFlight installation, store-binary smoke, and two-identity multiplayer proof are complete from canonical source `b8e16b24fa9d717225c531a02ef1dc4fb0806b85`. The remaining external delivery blocker is Google Play upload credentials for the already signed AAB. Keep SceneKit as reference material only.
