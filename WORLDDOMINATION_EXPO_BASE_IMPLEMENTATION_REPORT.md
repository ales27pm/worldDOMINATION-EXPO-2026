...
R3F is now the production-default renderer for campaign and multiplayer gameplay. Explicit `renderer=svg` or `renderer=2d` routes and the runtime error boundary retain the SVG implementation as the controlled fallback. The accepted qualification pair is complete and physical Android hardware is not required by the current contract. Store delivery remains separate from renderer qualification: a signed AAB upload, processing and TestFlight installation of the new default build, and a two-user authenticated multiplayer proof must still be completed before claiming end-to-end release delivery. Keep SceneKit as reference material only.
