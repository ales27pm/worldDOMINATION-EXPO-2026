...
- `pnpm test` currently runs both the compiled Node unit lane and Vitest lane, covering 119 Node tests and 2 Vitest tests.
...
The implemented release now includes the corrected China and mode-specific West Africa atlas, transparent map HUD, repaired renderer-neutral camera contract, deterministic indexed classic/expanded named-territory GLBs, a deterministic territory-name atlas rendered in one merged depth-aware R3F mesh, one shared renderer-boundary scene model consumed by both SVG and R3F, renderer-neutral camera intent, active-frame profiling, a physical-only performance qualification contract, and versioned persistent/exportable evidence, plus an opt-in R3F tabletop vertical slice with platform-specific asset loading and demand-driven native rendering. The exact source passes 119 Node tests, 2 Vitest cases, deterministic GLB and label-atlas validation, the complete browser smoke including classic 42/42 and expanded 48/48 labels, live same-revision `3D -> 2D -> 3D` switching, modal suspension, completed camera/battle profiles with browser ineligibility, evidence persistence/export, save/restore, and multiplayer snapshot presentation. Android-emulator and iOS-simulator runs prove real raycast attack interaction, covered-modal suspension, post-dismissal recovery, complete evidence persistence, and system sharing; current Android x86_64 compilation and SSH-only macOS TypeScript/Xcode validation cover the native build paths. The deployed Expo web alias was not changed. No new signed release or App Store Connect submission was produced for this final source patch. The signed Android versionCode 11 AAB and signed iOS buildNumber 11 IPA remain valid historical distribution evidence but do not contain the patch; App Store Connect accepted build 11 through submission `2781fb7c-1e31-4da1-adcb-f6e9bca60474`, but Apple processing and TestFlight installation are not claimed. The SVG map remains the production default because initial native Canvas framing is not yet visually deterministic, physical Android/iOS profiling is absent, and the Gate 4 parity/60 fps contract is still open. The private Tailscale deployment verifies loopback-only API hosting, upstream identity ownership, Postgres-backed account/contact discovery, invitation redaction, and deployed webhook notification delivery. The current release gate remains 45/47; external work still includes a Google Play service-account key for internal-track upload, physical Android and iOS R3F profiling, two physical devices or a second authenticated human account for end-to-end multiplayer proof, and TestFlight processing/installation verification. Keep SceneKit as reference material only.
