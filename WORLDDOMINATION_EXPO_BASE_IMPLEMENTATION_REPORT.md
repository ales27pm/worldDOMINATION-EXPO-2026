...
- Post-submission R3F contract hardening now builds one shared `MapSceneModel` at the renderer boundary and feeds that exact model to both SVG and R3F, with revisions covering every renderer-facing field. It also emits each newly observed battle presentation once, suppresses restored or repeated battle snapshots, resolves equal-depth border hits by canonical territory order, repairs native demand-render pick registration, and records renderer-neutral active camera/battle frame profiles against a fail-closed 60 Hz qualification contract. Browser and simulator runs can report metric results but are always ineligible to satisfy the target-device gate; only complete camera and battle profiles from hardware identified by `expo-device` as physical can pass. R3F territory-name parity comes from a deterministic 1024x1024 IM Fell English atlas rendered as one merged, depth-aware mesh; classic and expanded scenes report 42/42 and 48/48 labels respectively. The exact patch passes TypeScript, 115 Node tests, 2 Vitest tests, the complete Playwright/export matrix including a same-revision live `3D -> 2D -> 3D` switch and browser-ineligibility assertion, native Android compilation with `expo-device` 8.0.10 autolinked, real iOS 26.3 Simulator camera/battle qualification plus the earlier renderer-switch/tap/raycast/combat proof through the SSH macOS host, and fresh SSH/Xcode simulator compilation with `ExpoDevice` linked. The signed versionCode 11 AAB and buildNumber 11 IPA above predate this patch and remain historical distribution evidence; they are not claimed to contain the patch.
...
- `pnpm test` currently runs both the compiled Node unit lane and Vitest lane, covering 115 Node tests and 2 Vitest tests.
...
| Native iOS/Xcode simulator binary compile | Verified | Xcode work ran through `ssh -p 2222 ales27pm@127.0.0.1` on macOS with Xcode 26.3. The exact synchronized source passed remote Node 22 TypeScript validation, CocoaPods linked `ExpoDevice` 8.0.10, and `xcodebuild -workspace ios/worldDOMINATION.xcworkspace -scheme worldDOMINATION -configuration Debug -sdk iphonesimulator -destination "generic/platform=iOS Simulator" -derivedDataPath ios/build/DerivedData CODE_SIGNING_ALLOWED=NO DEBUG_INFORMATION_FORMAT=dwarf COMPILER_INDEX_STORE_ENABLE=NO build` ended with `** BUILD SUCCEEDED **`. The current output is `/Users/ales27pm/worlddomination-risk-scan-ios-build/artifacts/mobile/ios/build/DerivedData/Build/Products/Debug-iphonesimulator/worldDOMINATION.app`, 230,612 KiB, with a universal x86_64/arm64 simulator executable, bundle id `com.worlddomination.app`, and executable SHA-256 `a778084dd3cc68f1418b1004c5c709616bc6710d8a11c21e8d9ced0a1562d253`. A live simulator run published completed camera and battle frame profiles plus the required `simulator`/`ineligible` aggregate qualification. | This proves unsigned Debug simulator compilation and fail-closed simulator runtime qualification through SSH/Xcode, not signed physical-device install, physical 60 fps qualification, TestFlight, or App Store readiness. |
...
The exact final source passes TypeScript, 115 Node tests, 2 Vitest tests, deterministic GLB and territory-label atlas regeneration, and the Khronos validator with zero GLB errors or warnings. The checked-in label atlas is 250,218 bytes with SHA-256 `39a08d962e2eca221a99a3b3f6a0c306447b74e9a62a29f6a2e15880e775c11f`; its manifest records the exact font hash and canonical label indices. The complete Playwright browser matrix passes across portrait, landscape, and desktop with 97 game images. Its R3F routes prove scene readiness, 42/42 classic pick meshes and labels, 48/48 expanded pick meshes and labels, a nonblank WebGL canvas, the three army classes, camera zoom, completed camera and battle frame profiles, Brazil and North Africa raycasts, attack dispatch, finite effect completion, canonical save/restore without replay, and one-shot rendering of a changed multiplayer snapshot without replay on identical polling snapshots. It also requires `environment=browser`, `status=ineligible`, no missing profile kinds, and contract version 1, proving browser metrics cannot satisfy the physical gate. The browser switches live from R3F to SVG and back on the same game route, requires the identical shared scene revision and territory count in both renderers, verifies R3F unmount cleanup, and proves the remounted scene restores all 42 pick meshes and labels before continuing the interaction suite. The inspected classic browser frame is `.local/release-evidence/r3f-labels-web-classic-20260730.png`. Software SwiftShader is sufficient for functional smoke and qualification-contract validation but is not valid evidence for the Gate 4 60 fps target-device requirement.
...
The current native iOS source also compiles through the required SSH macOS/Xcode path. The exact synchronized patch passed remote TypeScript under Node 22, CocoaPods installed `ExpoDevice` 8.0.10, and the Xcode 26.3 generic iOS Simulator build exited 0. It produced a 230,612 KiB `worldDOMINATION.app` with a universal x86_64/arm64 executable, bundle `com.worlddomination.app` version `1.0.0`, and executable SHA-256 `a778084dd3cc68f1418b1004c5c709616bc6710d8a11c21e8d9ced0a1562d253`. The fresh compile log is `/tmp/worlddomination-frame-qualification-final-xcode-20260730.log` on the macOS guest. The SSH host still exposes only itself and simulators. The earlier signed local EAS production archive produced buildNumber 11 and submission `2781fb7c-1e31-4da1-adcb-f6e9bca60474`; App Store Connect accepted that earlier binary for processing, but it predates this patch.

The same source now has native iOS runtime evidence on the headless iPhone 16 / iOS 26.3 simulator reached exclusively through SSH. The rendered frame visibly contains the canonical atlas, extruded territories, instanced armies, transparent HUD, camera controls, and readable territory labels. CoreSimulator `idb` taps selected China through the real Gesture Handler/raycast path, exposed canonical targets, entered and resolved a battle, dismissed playback, and returned to the R3F map. Runtime inspection reported `modelName=Simulator iOS`, `modelId=x86_64`, and `environment=simulator`. The fresh camera profile passed metrics with 63 samples over 1,049.977 ms, 60.001 average FPS, 16.660 ms p50, 16.796 ms p95, 17.153 ms p99/max, zero estimated drops, and a 1.000 within-budget ratio. A deliberately slow QEMU battle transition produced one 5,841.610 ms event frame and correctly failed the metric contract instead of disappearing as an incomplete profile. With both reports present, the aggregate result was `metricStatus=fail`, `status=ineligible`, and no missing kinds. The ineligible result is mandatory for every simulator regardless of metrics. Earlier runtime inspection reported 42/42 registered picker meshes; a real `idb` tap switched R3F to SVG and back with an identical shared scene revision, then restored 42/42 pick meshes. Label validation reported classic 42/42/42 and fresh-storage expanded 48/48/48 territory/pick/label counts; inspected frames are `.local/release-evidence/r3f-labels-ios-classic-20260730.png` and `.local/release-evidence/r3f-labels-ios-expanded-20260730.png`. These are simulator functional, fail-closed qualification, and visual-parity evidence inside the QEMU macOS guest, not physical-iPhone or target-hardware evidence.
