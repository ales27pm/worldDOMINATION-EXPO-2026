...
3. **Complete for the vertical slice:** R3F selection, attack targeting, attention-directed camera, round-table presentation, three army models, finite battle animation, deterministic picking, save/restore, and multiplayer snapshot rendering pass web and Android native validation. Exact source `efcbb0a` passes the Android emulator interaction and performance slice; `0c136a5` independently passes it on physical iOS.
4. **Closed:** replace the SVG board only after visual parity and the versioned 60 fps qualification contract pass on a same-source accepted pair: Android emulator or hardware plus physical iOS. The Android emulator half is accepted and passing; the remaining renderer-gate work is a fresh physical-iOS run from the same source and canonical scene fixture.

## Final Recommendation

Continue with `worldDOMINATION_EXPO/artifacts/mobile` as the only production target for the consolidation.

The implemented release now includes the corrected China and mode-specific West Africa atlas, the same single-region China correction in R3F, transparent map HUD, deterministic indexed classic/expanded named-territory GLBs, one shared renderer-boundary scene model consumed by SVG and R3F, a merged territory-name atlas, renderer-neutral camera intent, active-frame profiling, a platform-specific release qualification contract, and versioned persistent/exportable evidence. The opt-in R3F path now seats the board inside the supplied round-table design and routes camera motion through cached territory bounds plus a deterministic Attention Director with coalescing, priority, TTL, duplicate suppression, automatic-move budgeting, gesture cancellation, a 2.5-second manual lease, reduced-motion/screen-reader behavior, and independent native announcements. Delayed tap recognition caches the original raycast result, so camera motion cannot reinterpret a rapid follow-up territory tap.

The current source passes TypeScript and 146 Node tests; the complete Playwright/export matrix passes with 152 files and 97 game images. The browser proof includes classic 42/42 and expanded 48/48 pick/label parity, a nonblank table scene, same-revision `3D -> 2D -> 3D` switching, a corrected continuous R3F China region, Brazil-to-North-Africa attack selection, modal suspension, camera/battle profiles, evidence persistence/export, save/restore, and multiplayer snapshot presentation. Exact source `864d9cf` adds a forced native Android release compile and visual China-parity smoke plus a file-identical SSH/Xcode Release simulator compile, bundle inspection, installation, and launch. Exact source `efcbb0a` remains the accepted Android emulator performance record with passing camera, battle, table, raycast, attack, zoom, and full-board evidence. Exact source `0c136a5` independently passes the physical-iPhone interaction and warmed performance slice, including visible table composition and deterministic China-to-Mongolia play. These records prove both platform halves separately, but they cannot be combined because their source and scene revisions differ.

The SVG map therefore remains the production default until the table/board composition and both performance profiles pass on a same-source accepted pair: Android emulator or hardware plus physical iOS. Android emulator validation is now accepted and the current Android half passes; no physical Android device is required for this renderer decision. The paired gate remains closed only because the retained passing physical-iOS record predates `efcbb0a` and uses a different canonical scene fixture. A fresh physical-iOS capture from the current exact source and fixture is required when the iPhone is available again. The deployed Expo alias was not changed. Exact source `0c136a5c57e5fd0d9bd68946fbf2244da29622ce` has a verified signed Android versionCode 12 AAB, a verified signed iOS buildNumber 14 store IPA accepted by App Store Connect through submission `a491d19e-871e-42ed-ac5e-0732cd41f6b1`, and a verified installed buildNumber 15 ad hoc IPA with passing physical-iOS evidence. Google Play upload, Apple processing completion, TestFlight availability/install, and a passing paired qualification are not claimed. The current release gate remains 45/47; external work still includes a Google Play service-account key, a current same-source physical-iOS qualification record, two physical devices or a second authenticated human account for end-to-end multiplayer proof, and TestFlight processing/installation verification. Keep SceneKit as reference material only.
