...
- A post-submission R3F contract hardening patch now makes scene revisions cover every renderer-facing field, emits each newly observed battle presentation once, suppresses restored or repeated battle snapshots, resolves equal-depth border hits by canonical territory order, and repairs native demand-render pick registration. The exact patch passes TypeScript, 104 Node tests, 2 Vitest tests, the complete Playwright/export matrix, native Android emulator raycast plus save/restore proof, and SSH/Xcode simulator compilation. The signed versionCode 11 AAB and buildNumber 11 IPA above predate this patch and remain historical distribution evidence; they are not claimed to contain the patch.
...
- `pnpm test` currently runs both the compiled Node unit lane and Vitest lane, covering 104 Node tests and 2 Vitest tests.
...
The exact final source passes TypeScript, 104 Node tests, 2 Vitest tests, deterministic GLB regeneration, and the Khronos validator with zero GLB errors or warnings. The complete Playwright browser matrix passes across portrait, landscape, and desktop with 97 game images. Its R3F routes prove scene readiness, 42/42 registered classic pick meshes, a nonblank WebGL canvas, the three army classes, camera zoom, Brazil and North Africa raycasts, attack dispatch, finite effect completion, canonical save/restore without replay, and one-shot rendering of a changed multiplayer snapshot without replay on identical polling snapshots. Software SwiftShader is sufficient for functional smoke but is not valid evidence for the Gate 4 60 fps target-device requirement.
...
3. **In progress:** the R3F selection, attack, camera, three-army-model, finite battle-animation, deterministic picking, save/restore, and multiplayer snapshot slice passes web plus Android-emulator interaction validation; validate the same slice on physical Android and iOS hardware.
