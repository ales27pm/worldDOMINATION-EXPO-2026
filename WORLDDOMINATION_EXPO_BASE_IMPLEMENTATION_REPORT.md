...
- The retained `/tmp/worlddomination-android-evidence-final.json` is no longer rejected merely because it came from an emulator. It still cannot open the gate: it has no native build version, uses the non-SHA source marker `working-tree-production-evidence`, differs from the physical-iOS canonical fixture, and both camera and battle profiles fail the version 1 thresholds. A fresh same-source Android emulator capture with passing profiles is therefore still required.
...
- The cross-platform handoff is an executable fail-closed release gate. EAS qualification builds require committed source and embed the exact full Git revision through `EAS_BUILD_GIT_COMMIT_HASH`; direct local and simulator builds retain an unqualified `null` fallback. `pnpm run map:release:check` accepts one parsed evidence file per platform and passes only when recent Android emulator-or-hardware evidence and physical-iOS evidence use the expected commit, the same application version and canonical scene fixture, distinct runtime sessions, credible environment-specific device/OS identities, correct 42/48 territory counts, and passing camera plus battle profiles. The old `map:physical:check` command is retained as an exact compatibility alias. Existing Android simulator evidence is accepted by environment policy but rejected for source/build provenance, metrics, and fixture mismatch. Exact source `0c136a5c57e5fd0d9bd68946fbf2244da29622ce` has a passing physical-iOS record but no matching passing Android record, so the paired gate still exits nonzero and SVG remains the production default.
...
The SSH macOS build host is reachable with macOS 15.7.7 and Xcode 26.3, and the real iPhone completed pairing. During qualification Linux enumerated Apple USB device `05ac:12a8` with serial suffix `C0801C`, QEMU owned the live USB node through stable passthrough, and `xcrun devicectl list devices` reported an available paired iPhone 16 Pro. Exact-source buildNumber 15 was signed, installed, launched, screen-captured, and driven through a dedicated XCTest runner. Real raycast selection, save/continue, attack targeting, finite battle presentation, zoom controls, and full-board table framing all completed. The final persisted physical iOS evidence passes both 60 Hz profiles. After evidence capture QEMU logged `NO_DEVICE`; the current live recheck finds no Apple USB device and CoreDevice lists only the cached iPhone as unavailable. No matching passing Android qualification record is currently supplied, so `pnpm run map:release:check` correctly exits 1 when only the iOS record is supplied; Android hardware is no longer required because a conforming emulator record is accepted. Current repository HEAD `03cf20249d8f06f651e0963a9b00518511a1511d` adds the deterministic qualification launch after the profiled `0c136a5` revision; any future release-pair decision must use evidence from one exact source revision rather than pairing records across those revisions. Gate 4 is not open.
...
3. **Complete for the vertical slice:** R3F selection, attack targeting, attention-directed camera, round-table presentation, three army models, finite battle animation, deterministic picking, save/restore, and multiplayer snapshot rendering pass web and Android native validation; exact source `0c136a5` also passes the functional and performance slice on physical iOS.
4. **Closed:** replace the SVG board only after visual parity and the versioned 60 fps qualification contract pass on a same-source accepted pair: Android emulator or hardware plus physical iOS. The remaining Android emulator interaction checks and the complete browser/native regression matrix must also pass.

## Final Recommendation

Continue with `worldDOMINATION_EXPO/artifacts/mobile` as the only production target for the consolidation.

The implemented release now includes the corrected China and mode-specific West Africa atlas, transparent map HUD, deterministic indexed classic/expanded named-territory GLBs, one shared renderer-boundary scene model consumed by SVG and R3F, a merged territory-name atlas, renderer-neutral camera intent, active-frame profiling, a platform-specific release qualification contract, and versioned persistent/exportable evidence. The opt-in R3F path now seats the board inside the supplied round-table design and routes camera motion through cached territory bounds plus a deterministic Attention Director with coalescing, priority, TTL, duplicate suppression, automatic-move budgeting, gesture cancellation, a 2.5-second manual lease, reduced-motion/screen-reader behavior, and independent native announcements. Delayed tap recognition caches the original raycast result, so camera motion cannot reinterpret a rapid follow-up territory tap.

The current source passes TypeScript and 143 Node tests; the earlier exact-source browser, asset, Android compile, and SSH/Xcode evidence remains recorded above. The browser proof includes classic 42/42 and expanded 48/48 pick/label parity, a nonblank table scene, same-revision `3D -> 2D -> 3D` switching, Brazil-to-North-Africa attack selection, modal suspension, camera/battle profiles, evidence persistence/export, save/restore, and multiplayer snapshot presentation. Exact source `0c136a5` additionally passes the physical iPhone interaction and warmed performance slice, including visible table composition and deterministic China-to-Mongolia play. EAS build provenance and the release-pair CLI make the remaining Android emulator decision reproducible and auditable. A passing same-source Android emulator record now substitutes for Android hardware; the historical emulator record remains insufficient because its provenance and profile metrics fail.

The SVG map therefore remains the production default until the table/board composition and both performance profiles pass on a same-source accepted pair: Android emulator or hardware plus physical iOS. The physical-iPhone half passes; Android hardware is explicitly not required for now. A fresh matching Android emulator record is still needed because the retained capture fails provenance, fixture, and metric checks. The deployed Expo alias was not changed. Exact source `0c136a5c57e5fd0d9bd68946fbf2244da29622ce` has a verified signed Android versionCode 12 AAB, a verified signed iOS buildNumber 14 store IPA accepted by App Store Connect through submission `a491d19e-871e-42ed-ac5e-0732cd41f6b1`, and a verified installed buildNumber 15 ad hoc IPA with passing physical iOS evidence. Google Play upload, Apple processing completion, TestFlight availability/install, and a passing paired qualification are not claimed. The current release gate remains 45/47; external work still includes a Google Play service-account key, a current same-source Android qualification record with emulator accepted, two physical devices or a second authenticated human account for end-to-end multiplayer proof, and TestFlight processing/installation verification. Keep SceneKit as reference material only.
