...
- Physical-device enumeration remains empty: local ADB exposes only the Android emulator, the Linux host has no mobile USB device, and the SSH macOS host lists only itself plus simulators. Only one human tailnet identity is currently connected, so recipient-side proof from a second authenticated account is also unavailable. No physical or two-user claim is made.
...
- The physical handoff is now an executable fail-closed gate. EAS qualification builds require a committed source and embed the exact full Git revision through `EAS_BUILD_GIT_COMMIT_HASH`; direct local and simulator builds retain an unqualified `null` fallback. `pnpm run map:physical:check` accepts one parsed evidence file per platform and passes only when recent physical Android and iOS captures use the expected commit, the same application version and canonical scene fixture, distinct runtime sessions, credible device/OS identities, correct 42/48 territory counts, and passing camera plus battle profiles. The existing Android and iOS simulator evidence is intentionally rejected for environment, provenance, metrics where applicable, and fixture mismatch. No physical-device pass is claimed, and SVG remains the production default.
...
The SSH macOS build host is reachable with macOS 15.7.7 and Xcode 26.3, and a real iPhone is now physically present. Linux enumerates Apple USB device `05ac:12a8` with serial suffix `C0801C`; QEMU owns the live USB node through stable vendor/product passthrough, and macOS `system_profiler SPUSBDataType` sees the same iPhone at 480 Mb/s. `xcrun devicectl list devices`, `xcrun xctrace list devices`, and `xcrun xcdevice list` still expose no physical device, which is consistent with an unresolved unlock/`Trust This Computer` pairing handshake. No app install, runtime frame profile, or exported physical evidence has therefore been obtained. The SVG renderer remains the production default and Gate 4 is not open.
...
3. **In progress:** the R3F selection, attack, attention-directed camera, round-table presentation, three-army-model, finite battle-animation, deterministic picking, save/restore, and multiplayer snapshot slice passes web plus Android/iOS simulator validation; validate visual parity, interaction, and the same performance profiles on physical Android and iOS hardware.
4. Replace the SVG board only after visual parity, the versioned 60 fps qualification contract passes camera and battle workloads on physical Android and iOS hardware, the remaining physical-device interaction checks pass, and the complete browser/native regression matrix remains green.
...
The exact source passes TypeScript, 137 Node tests, 2 Vitest cases, deterministic asset validation, and the complete Playwright/export matrix with 152 exported files and 97 game images. The browser proof includes classic 42/42 and expanded 48/48 pick/label parity, a nonblank table scene, same-revision `3D -> 2D -> 3D` switching, Brazil-to-North-Africa attack selection, modal suspension, camera/battle profiles, evidence persistence/export, save/restore, and multiplayer snapshot presentation. The final local Android x86_64 build and the file-identical SSH/Xcode simulator build both pass. EAS build provenance and the paired physical-evidence CLI now make the remaining hardware decision reproducible and auditable. Simulator profiles and captures remain diagnostic only: Android's table profile misses strict p95/within-budget thresholds, iOS metrics are simulator-ineligible, and CoreSimulator's frozen GL capture prevents current iOS visual-parity proof.

The SVG map therefore remains the production default until the table/board composition is inspected on real iPhone/iPad and Android hardware and both physical camera and battle profiles pass Gate 4. The deployed Expo alias was not changed. Exact source `0c136a5c57e5fd0d9bd68946fbf2244da29622ce` now has a verified signed Android versionCode 12 AAB and a verified signed iOS buildNumber 14 IPA; App Store Connect accepted build 14 through submission `a491d19e-871e-42ed-ac5e-0732cd41f6b1` and started processing it. Google Play upload, Apple processing completion, TestFlight availability/installation, and physical-device R3F evidence are not claimed. The current release gate remains 45/47; external work still includes a Google Play service-account key, physical Android/iOS R3F proof, two physical devices or a second authenticated human account for end-to-end multiplayer proof, and TestFlight processing/installation verification. Keep SceneKit as reference material only.
