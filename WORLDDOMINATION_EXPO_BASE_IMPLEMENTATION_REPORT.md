...
- A fresh local EAS Android production build completed under `ales27pm` with remote credential `Build Credentials AcpOI7ZatM`, application ID `com.worlddomination.app`, versionCode `7`, and versionName `1.0.0`. Store bundle `.local/eas-builds/worlddomination-production-current-20260730.aab` is 73,362,967 bytes with SHA-256 `e9950ce8568958b9070d3ab8555fbc8e7fe5d4e3d6cf1fc490504d6e0da0a07a`; `jarsigner -verify` exits 0, and its signer SHA-256 is `97:7D:8A:82:C3:BF:95:F8:54:DB:AE:4A:50:2E:A4:D6:FB:3F:F7:B8:07:AC:A8:A0:3A:D9:57:95:44:AE:D8:D8`. Bundletool confirms versionCode 7, and the embedded release bundle contains the China, West Africa, and current API markers.
...
- A fresh post-atlas signed iOS production build completed through SSH/Xcode 26.3 with EAS buildNumber `9`. IPA `.local/eas-builds/worlddomination-production-ios-current-20260730.ipa` is 24,308,396 bytes with SHA-256 `bdd140cb1bac45a1f0f4bb2518b8d67ae644548e1c3027787fda2cf69cfc4a92`; strict deep code-sign verification passes for bundle `com.worlddomination.app`, version `1.0.0`, team `52T7P32J34`, and the active App Store provisioning profile. Its release bundle contains the China atlas marker, `W. Africa`, and the private Tailscale API host. EAS submission `f90b11af-3124-45e1-8a68-ac9bfd2b93d9` is `FINISHED` with no error for App Store Connect app `6796113966`, and Apple accepted the binary for processing. Evidence is `.local/release-evidence/ios-production-build-20260730.json` and `.local/release-evidence/eas-ios-submission-status-20260730.json`; TestFlight processing or physical installation is not claimed.
...
- `pnpm test` currently runs both the compiled Node unit lane and Vitest lane, covering 87 Node tests and 2 Vitest tests.
...
- The Expo project remains linked to `@ales27pm/worlddomination` with project ID `f9205fa6-ad0e-4808-877d-a8f4cf1856fb`. Historical local EAS Android preview/production builds and the signed iOS build remain recorded below. The currently attached EAS token authenticates to another owner and is not authorized to read this project, so it cannot be used for a fresh credential-backed local EAS Android build or project query. No Replit target is used.
...
| Native Android debug binary compile | Verified | `ANDROID_HOME=/home/ales27pm/.local/android-tooling/android-sdk-1785090272 ANDROID_SDK_ROOT=/home/ales27pm/.local/android-tooling/android-sdk-1785090272 JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64 ./gradlew :app:assembleDebug` completed successfully from `artifacts/mobile/android`. The current debug APK is `artifacts/mobile/android/app/build/outputs/apk/debug/app-debug.apk` at 195 MB. | This proves Android debug compilation, separate from the local EAS release APK evidence below. |
| Native iOS/Xcode simulator binary compile | Verified | Xcode work ran through `ssh -p 2222 ales27pm@127.0.0.1` on macOS with Xcode 26.3. The remote build copy ran `xcodebuild -workspace ios/worldDOMINATION.xcworkspace -scheme worldDOMINATION -configuration Debug -sdk iphonesimulator -destination "generic/platform=iOS Simulator" -derivedDataPath ios/build/DerivedData CODE_SIGNING_ALLOWED=NO build` and ended with `** BUILD SUCCEEDED **`. The current output is `/Users/ales27pm/worlddomination-risk-scan-ios-build/artifacts/mobile/ios/build/DerivedData/Build/Products/Debug-iphonesimulator/worldDOMINATION.app`, 213 MB, with a universal x86_64/arm64 simulator executable and bundle id `com.worlddomination.app`. | This proves unsigned Debug simulator compilation through SSH/Xcode, not signed physical-device install, TestFlight, App Store readiness, or latest-run simulator launch. |
...
The latest successful Android debug artifact is `worldDOMINATION_EXPO/artifacts/mobile/android/app/build/outputs/apk/debug/app-debug.apk`.
...
`scripts/browser-smoke.js` now checks the exact computed alpha values for the passive, control, roster, and dispatch surfaces as well as the camera controls. The full rendered smoke passes across portrait, landscape, and desktop, including 97 game images. TypeScript, 91 Node unit tests, 2 Vitest tests, and `git diff --check` pass. Review screenshots are stored in `.local/transparent-map-hud-20260729/`.
...
- `assets/game/map-3d/world-map-classic.glb`: 42 meshes, merged classic North Africa, 1,663,540 bytes, SHA-256 `9f46d17bb9beb3aa6476f4ace5015287e5306560e27db9206f3947f76a00db10`.
- `assets/game/map-3d/world-map-expanded.glb`: 48 meshes, separate West Africa and all six optional territories, 1,722,396 bytes, SHA-256 `68f4ae7ef1082ea2d7b19a9a1500f9a03de3f3a8ba6fc3050a56e0231addae6f`.
...
The first renderer-neutral Gate 3 slice is implemented behind an explicit `2D`/`3D` control. The SVG map remains the default and `GameMap` retains an error-boundary fallback. The new path includes:

- `game/mapSceneModel.ts`, which derives stable territory mesh identities, ownership, army composition, view-mode tints, capitals, interactions, and deterministic battle effects from canonical `GameState`.
...
The exact final source passes TypeScript, 99 Node tests, 2 Vitest tests, deterministic GLB regeneration, and the Khronos validator with zero GLB errors or warnings. The complete Playwright browser matrix passes across portrait, landscape, and desktop with 97 game images. Its R3F route proves scene readiness, a nonblank WebGL canvas, the three army classes, camera zoom, Brazil and North Africa raycasts, attack dispatch, and the on-map battle effect. Software SwiftShader is sufficient for functional smoke but is not valid evidence for the Gate 4 60 fps target-device requirement.

The final Hermes Android release compile also succeeds locally. `:app:assembleRelease` completed 565 tasks and produced `android/app/build/outputs/apk/release/app-release.apk`, 113,732,962 bytes, SHA-256 `76896b9de8eadd98b53d5c51e28c19a9d5e11752e5fe422b32803462d72b189a`. APK Signature Scheme v2 verification passes; this direct Gradle artifact uses the generated debug certificate and is compile/package evidence, not the store-signed EAS AAB. The APK contains Hermes and Expo GL for all four Android ABIs, a 5,159,560-byte application bundle, and both canonical GLBs at their exact 1,663,540-byte and 1,722,396-byte sizes.

Android runtime promotion remains blocked. On the available Android 15 x86_64 SwiftShader emulator, the default SVG release route remains foreground, but opening the R3F route causes a native Hermes `SIGSEGV` on the JavaScript thread before Expo GL reports a context. Replacing the full board with a single R3F Canvas containing one box reproduces the same crash, ruling out the map GLBs, atlas, camera, raycast, army, and battle layers. R3F 9.4.2, React Compiler off, and an alternate native gesture surface did not change the result; those diagnostics were removed and the production source is restored to R3F 9.6.1, React Compiler, Hermes, and the tested Gesture Handler contract.
...
3. **In progress:** the R3F selection, attack, camera, three-army-model, and battle-animation slice passes web and Android compilation/package checks; resolve the Android x86_64 native Canvas crash and validate the same slice on physical Android and iOS hardware.
4. Replace the SVG board only after visual parity, 60 fps target-device profiling, deterministic hit testing, save/restore, multiplayer snapshot rendering, and browser/native regression checks pass.

## Final Recommendation

Continue with `worldDOMINATION_EXPO/artifacts/mobile` as the only production target for the consolidation.

The implemented release now includes the corrected China and mode-specific West Africa atlas, transparent map HUD, repaired renderer-neutral camera contract, deterministic classic/expanded named-territory GLBs, the renderer-neutral scene model and camera intent, and an opt-in R3F tabletop vertical slice. The exact source passes 99 Node tests, 2 Vitest cases, deterministic GLB validation, the complete browser smoke including R3F interaction, and a fresh Hermes Android release compile/package check. The SVG map remains the production default because the available x86_64 Android emulator crashes inside a minimal native R3F Canvas and no physical iPhone is attached for Gate 3 validation. The production Expo web alias and signed Android versionCode 10 AAB remain the current deployed/store Android evidence. The previously signed iOS buildNumber 9 IPA and its `FINISHED` App Store Connect submission remain the current iOS store-distribution evidence; the simulator compile does not replace or resubmit that IPA. The private Tailscale deployment verifies loopback-only API hosting, upstream identity ownership, Postgres-backed account/contact discovery, invitation redaction, and deployed webhook notification delivery. The current release gate remains 45/47; external work still includes a Google Play service-account key for internal-track upload, physical Android and iOS R3F profiling, two physical devices or a second authenticated human account for end-to-end multiplayer proof, and TestFlight processing/installation verification for buildNumber 9. Keep SceneKit as reference material only.
