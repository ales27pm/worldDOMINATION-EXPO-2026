...
As of the current 2026-07-30 working tree, the EXPO base has advanced beyond the initial consolidation plan. The implementation remains centered on `worldDOMINATION_EXPO/artifacts/mobile`; donor provenance and frozen revisions are recorded in `docs/CONSOLIDATION.md`, and the donors are historical remotes rather than alternate production targets.
...
- The physical handoff is now an executable fail-closed gate. EAS qualification builds require a committed source and embed the exact full Git revision through `EAS_BUILD_GIT_COMMIT_HASH`; direct local and simulator builds retain an unqualified `null` fallback. `pnpm run map:physical:check` accepts one parsed evidence file per platform and passes only when recent physical Android and iOS captures use the expected commit, the same application version and canonical scene fixture, distinct runtime sessions, credible device/OS identities, correct 42/48 territory counts, and passing camera plus battle profiles. The existing simulator evidence is intentionally rejected for environment, provenance, metrics where applicable, and fixture mismatch. Exact source `0c136a5c57e5fd0d9bd68946fbf2244da29622ce` now has a passing physical-iOS record, but no physical Android record; the paired gate therefore still exits nonzero and SVG remains the production default.
...
The version 1 performance contract targets 60 fps and requires both camera and battle profiles. Each profile requires at least 60 samples and 900 ms duration, average FPS at least 58.5, p95 no more than 18.333 ms, p99 no more than 33.333 ms, estimated dropped-frame ratio no more than 1%, and at least 95% of samples within the 16.667 ms frame budget. Deterministic failures identify invalid metrics, target mismatch, insufficient samples or duration, low average FPS, p95/p99 excess, dropped-frame excess, and within-budget deficit. Qualification remains `pending` until both kinds exist, becomes `ineligible` on browser or simulator, and can return `pass` or `fail` only on physical hardware.

`game/mapPhysicalQualification.ts` now composes those per-device results into one versioned Android+iOS release decision. It validates exact full-SHA source provenance, application/build/session provenance, credible physical-device identity, canonical territory counts, same-version and byte-identical scene revision pairing, evidence age, capture skew, and both required profile assessments. `scripts/verify-map-physical-qualification.ts` exposes the decision as a CI-friendly JSON command with a nonzero exit on failure. `scripts/write-build-source-revision.cjs`, invoked by the EAS post-install hook, embeds the official build commit without committing a stale SHA to source. Running the pair verifier against `/tmp/worlddomination-android-evidence-final.json` and `/tmp/worlddomination-ios-evidence.json` exits 1 as designed because these records are simulator evidence from older/different source fixtures.
...
The SSH macOS build host is reachable with macOS 15.7.7 and Xcode 26.3, and the real iPhone completed pairing. During qualification Linux enumerated Apple USB device `05ac:12a8` with serial suffix `C0801C`, QEMU owned the live USB node through stable passthrough, and `xcrun devicectl list devices` reported an available paired iPhone 16 Pro. Exact-source buildNumber 15 was signed, installed, launched, screen-captured, and driven through a dedicated XCTest runner. Real raycast selection, save/continue, attack targeting, finite battle presentation, zoom controls, and full-board table framing all completed. The final persisted physical iOS evidence passes both 60 Hz profiles. After evidence capture QEMU logged `NO_DEVICE`; the current live recheck finds no Apple USB device and CoreDevice lists only the cached iPhone as unavailable. ADB still lists no physical Android, so `pnpm run map:physical:check` correctly exits 1 when only the iOS record is supplied. Current repository HEAD `03cf20249d8f06f651e0963a9b00518511a1511d` adds the deterministic qualification launch after the profiled `0c136a5` revision; any future release-pair decision must rebuild both platforms from one exact current commit rather than pairing records across those revisions. Gate 4 is not open.
...
4. **Closed:** replace the SVG board only after visual parity and the versioned 60 fps qualification contract pass on a same-source physical Android+iOS pair, the remaining physical Android interaction checks pass, and the complete browser/native regression matrix remains green.

## Final Recommendation

Continue with `worldDOMINATION_EXPO/artifacts/mobile` as the only production target for the consolidation.

The implemented release now includes the corrected China and mode-specific West Africa atlas, transparent map HUD, deterministic indexed classic/expanded named-territory GLBs, one shared renderer-boundary scene model consumed by SVG and R3F, a merged territory-name atlas, renderer-neutral camera intent, active-frame profiling, a physical-only qualification contract, and versioned persistent/exportable evidence. The opt-in R3F path now seats the board inside the supplied round-table design and routes camera motion through cached territory bounds plus a deterministic Attention Director with coalescing, priority, TTL, duplicate suppression, automatic-move budgeting, gesture cancellation, a 2.5-second manual lease, reduced-motion/screen-reader behavior, and independent native announcements. Delayed tap recognition caches the original raycast result, so camera motion cannot reinterpret a rapid follow-up territory tap.

The exact source passes TypeScript, 137 Node tests, 2 Vitest cases, deterministic asset validation, and the complete Playwright/export matrix with 152 exported files and 97 game images. The browser proof includes classic 42/42 and expanded 48/48 pick/label parity, a nonblank table scene, same-revision `3D -> 2D -> 3D` switching, Brazil-to-North-Africa attack selection, modal suspension, camera/battle profiles, evidence persistence/export, save/restore, and multiplayer snapshot presentation. The final local Android x86_64 build and the file-identical SSH/Xcode simulator build both pass. Exact source `0c136a5` additionally passes the physical iPhone interaction and warmed performance slice, including visible table composition and deterministic China-to-Mongolia play. EAS build provenance and the paired physical-evidence CLI make the remaining Android hardware decision reproducible and auditable. Android simulator profiles remain diagnostic only and do not substitute for target hardware.

The SVG map therefore remains the production default until the table/board composition and both performance profiles pass on same-source physical Android and iOS builds. The iPhone half now passes; Android hardware remains missing. The deployed Expo alias was not changed. Exact source `0c136a5c57e5fd0d9bd68946fbf2244da29622ce` has a verified signed Android versionCode 12 AAB, a verified signed iOS buildNumber 14 store IPA accepted by App Store Connect through submission `a491d19e-871e-42ed-ac5e-0732cd41f6b1`, and a verified installed buildNumber 15 ad hoc IPA with passing physical iOS evidence. Google Play upload, Apple processing completion, TestFlight availability/install, physical Android R3F evidence, and the paired physical qualification are not claimed. The current release gate remains 45/47; external work still includes a Google Play service-account key, a current same-source physical Android/iOS pair, two physical devices or a second authenticated human account for end-to-end multiplayer proof, and TestFlight processing/installation verification. Keep SceneKit as reference material only.
