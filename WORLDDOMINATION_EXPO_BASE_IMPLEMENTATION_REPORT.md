...
3. Build an R3F vertical slice for selection, attack targeting, camera control, three army models, and one battle animation on web, Android, and physical iOS.
...
The implemented release now includes the corrected China and mode-specific West Africa atlas, transparent map HUD, repaired renderer-neutral camera contract, deterministic classic/expanded named-territory GLBs, 91 passing Node tests plus 2 Vitest cases, rendered browser smoke, a production-alias Playwright interaction proof, production Expo web alias, signed Android versionCode 10 AAB, and a fresh successful SSH/Xcode universal simulator compile. The previously signed iOS buildNumber 9 IPA and its `FINISHED` App Store Connect submission remain the current store-distribution evidence; the simulator compile does not replace or resubmit that IPA. The private Tailscale deployment verifies loopback-only API hosting, upstream identity ownership, Postgres-backed account/contact discovery, invitation redaction, and deployed webhook notification delivery. Gate 3 is next: use React Three Fiber to consume the shared geometry contract for selection, attack targeting, renderer-neutral camera control, three army models, and one battle animation on web, Android, and physical iOS before replacing the SVG board. The current release gate remains 45/47; the external steps are supplying a Google Play service-account key for internal-track upload, attaching two physical devices or a second authenticated human account for end-to-end multiplayer proof, and verifying TestFlight processing/installation for buildNumber 9. Keep SceneKit as reference material only.
