...
- `pnpm test` currently runs both the compiled Node unit lane and Vitest lane, covering 100 Node tests and 2 Vitest tests.
...
`scripts/browser-smoke.js` now checks the exact computed alpha values for the passive, control, roster, and dispatch surfaces as well as the camera controls. The full rendered smoke passes across portrait, landscape, and desktop, including 97 game images. TypeScript, 100 Node unit tests, 2 Vitest tests, and `git diff --check` pass. Review screenshots are stored in `.local/transparent-map-hud-20260729/`.
...
The prior Android x86_64 native Canvas crash is resolved. On the available Android 15 x86_64 SwiftShader emulator, the actual canonical R3F board rendered and remained foreground while `Full board`, Brazil selection, North Africa targeting, attack dispatch, and the on-map battle/occupation flow were exercised through native raycasts. A three-minute demand-render idle run stabilized around 328 MB native allocation. A subsequent five-minute post-battle endurance run retained the same process ID across twenty 15-second samples and settled within an 88 KB native-allocation band around 334 MB after the finite effect ended. This closes the emulator interaction/crash blocker; it does not substitute for physical-device frame profiling.
...
The current native iOS source also compiles through the required SSH macOS/Xcode path. After `expo prebuild` and CocoaPods installed Expo GL 16.0.10, the exact Xcode 26.3 iOS Simulator build exited 0 and produced a 114,776 KiB `worldDOMINATION.app` for x86_64, bundle `com.worlddomination.app` version `1.0.0`, with executable SHA-256 `4805a37570292bbc3a0c4e27ad101809695cbe4bd6368c765faddbdb76432a15`. The signed local EAS production archive then produced buildNumber 11 and submission `2781fb7c-1e31-4da1-adcb-f6e9bca60474`; App Store Connect accepted the binary for processing.
...
3. **In progress:** the R3F selection, attack, camera, three-army-model, and finite battle-animation slice passes web and Android compilation/package checks plus Android-emulator interaction and endurance validation; validate the same slice on physical Android and iOS hardware.
